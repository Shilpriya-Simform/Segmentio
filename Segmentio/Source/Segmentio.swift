...
open class Segmentio: UIView {
    ...
    open func changeTitle(at index: Int, title: String?) {
        if segmentioItems.count > index {
        segmentioItems[index].changeTitle(title)
        segmentioCollectionView?.reloadData()
        }
    }
    ...
    private func setupHorizontalSeparator() {
        ...
        if type == .bottom || type == .topAndBottom {
            ...
            setupConstraintsForSeparatorView(
                ...
                originY: frameForSegmentCollectionView().height - height
